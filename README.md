...
トップページでは1週間分の予約状況を会議室ごとに俯瞰でき、各日の詳細表示、予約の登録・編集・削除、毎週予約、会議室設備情報の登録・閲覧を行えます。
...
- 1週間分の予約状況を会議室ごとに表示
- 曜日クリックによる日別予約一覧への切り替え
- 会議室 × 日付のマスをクリックして新規予約
- 会議室 × 日付のマスにマウスオーバーして、その日・その部屋の予約一覧をポップアップ表示
- 週表示の会議室名にマウスオーバーして、定員・設備・注意事項をポップアップ表示
- 日ごとの会議室別予約一覧
- 前週・翌週、今日、カレンダーによる日付移動
- 予約の登録、編集、削除
- 予約時間の重複チェック
- 毎週予約
...
```text
https://example.com/room_book/setup-web.php
```
...
```text
https://example.com/room_book/
```

または、

```text
https://example.com/room_book/index.php
```

## トップページの使い方

トップページの上部には、表示中の日付を含む1週間分の予定表が表示されます。

- 行：会議室
- 列：月曜日から日曜日
- マス：その会議室・その日の予約状況

曜日の見出しをクリックすると、その日の詳細な予約一覧に切り替わります。

会議室 × 日付のマスをクリックすると、その日・その部屋の新規予約画面を開きます。

PCでは、会議室 × 日付のマスにマウスオーバーすると、その日・その部屋の予約一覧がポップアップ表示されます。ポップアップ内の予約タイトルをクリックすると、予約編集画面を開きます。

また、左列の会議室名にマウスオーバーすると、その会議室の定員・設備・注意事項がポップアップ表示されます。

スマートフォンではマウスオーバーがないため、各リンクやマスをタップして操作します。

## 会議室詳細情報

予約一覧の「設備・詳細」または週表示の会議室名から、各会議室の情報を確認できます。
...
```text
https://example.com/room_book/data/reservation.sqlite
```
...
実在する会議室名や内部情報を公開したくない場合は、`config.php` をGit管理から除外し、代わりに `config.example.php` を用意してください。
